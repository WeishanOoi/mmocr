...
class SDMGR(BaseModel):
    ...
    def predict(self, inputs: torch.Tensor, data_samples: Sequence[KIEDataSample], **kwargs) -> List[KIEDataSample]:
        ...
        for prediction in predictions:
            # Access recognized entities and their labels based on the correct attributes or methods of KIEDataSample
            entities = prediction.get_recognized_entities()
            labels = prediction.get_entity_labels()
    
            # Perform further processing and combination of recognized entities
            combined_entities = {}  # Update this based on your data structure
            ...
            prediction.set_recognized_entities([formatted_entity])
            prediction.set_entity_labels(['Person'])
